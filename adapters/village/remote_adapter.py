"""Remote execution backend for the village model — the colo-node adapter.

Same four-method contract as :class:`VillageAdapter`; only ``run`` differs. It
ships the config to an execution environment, runs the solver there, captures
stdout to a local ``solver.log``, and pulls the result CSVs back. The framework,
skills, and guardrail are unchanged — this is purely a different ``run()``
backend, and it doubles as the Task-2 "submit to HPC" piece.
...
Assumes the execution environment has Julia + a Gurobi licence and a checkout of
the village model at ``remote_root`` (``ensure_remote_repo`` clones it if not).
"""
...
VILLAGE_REPO_URL = "https://github.com/Power-Lab/village-indonesia-100gw.git"
VILLAGE_BRANCH = "solar-resource-assessment"


class RemoteVillageAdapter(VillageAdapter):
    name = "village-remote"
    ...
    def __init__(
        ...
        repo_url: str = VILLAGE_REPO_URL,
        branch: str = VILLAGE_BRANCH,
    ):
    ...
    @classmethod
    def local_loopback(cls, remote_root: str, julia: str = "julia") -> "RemoteVillageAdapter":
    ...
    def ensure_remote_repo(self):
        ...
        cmd = (
            ...
            f"git clone -b {shlex.quote(self.branch)} {shlex.quote(self.repo_url)} {root} && "
            f"julia --project={root} {root}/bootstrap.jl; "
    ...
    def run(self, config: Dict[str, Any], run_dir: Path) -> Execution:
        ...
        # 2) run the solver in the execution environment
        run_cmd = (
            f"cd {shlex.quote(self.remote_root)} && "
            f"{self.julia} --project={shlex.quote(self.remote_root)} run_model.jl "
            f"--config {shlex.quote(remote_cfg)}"
        )
        ...
        self.transport.pull(self._remote_results_dir(config) + "/", str(out_dir) + "/")
